...
// The code relies entirely on the current data state, not the DOM state
...
export default (elements, initState, i18next) => {
  ...
  const handleLoadingProcessStatus = (state) => {
    ...
    // Logic relies on a process state and not on DOM flags or data
    // https://hexlet.io/courses/js-frontend-architecture/lessons/processes/theory_unit
    switch (loadingProcess.status) {
      case "failed":
        submit.disabled = false;
        input.removeAttribute("readonly");
        feedback.classList.add("text-danger");
        feedback.textContent = i18next.t([
  ...
  const handlePosts = (state) => {
    ...
    const postsListItems = posts.map((post) => {
      ...
      const className = ui.seenPosts.has(post.id)
  ...
  const handleModal = (state) => {
    ...
    // Do not insert third-party data via innerHTML
